...
//extern float windowWidth = 800;
//extern float windowHeight = 600;
extern bool run;
...
Shape::Shape(Vec2f c, Vec2f v, Vec2f a, int i) : center(c), velocity(v), acceleration(a), id(i) {
	//std::cout << "center in Shape constructor: " << center.x << " " << center.y << "\n";
}
...
* I'm sorry that it looks ugly, i tried
*/
...
/**
* Updates position of square, includes interaction with floor
*/
void Square::update() {
		...
		for (auto& square : vecSquares) {
			...
			for (int i = 0; i < 16; i += 4) {
				...
				if (distance <= circle->radius) {
					...
					fakeCircle->mass = circle->mass * 0.5f; // The mass is also a hack, this mass seems to make it relatively nice looking
...
bool Circle::del(double x, double y) {
